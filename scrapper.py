...
class Scrapper:
    weeks = range(1, conf["marathon_duration"] + 1)
    ...
    async def parse_daily_ration(self, content, week, day):
        ...
        for ind, meal in enumerate(parsed_ration):
            title = self.get_recipe_title(meal)
            ingredients = self.get_recipe_ingredients(meal)
            cook = self.get_recipe_cooking(meal)
            pic = await self.save_recipe_picture(meal)
            row = (week, day, self.eating[ind], title, ingredients, cook, pic)
    ...
    async def save_all_recipes(self):
        async with aiosqlite.connect(conf["db_name"]) as conn:
            await conn.execute("""
                ...
                (week integer, day integer, eating string, title text,
    ...
    @staticmethod
    def get_recipe_title(meal):
        return meal.find("div", class_="food_item_name").text

    @staticmethod
    def get_recipe_ingredients(meal):
        ingredients_divs = meal.find_all("td", class_="ingredients")
        return "\n".join([ingr.text for ingr in ingredients_divs])

    @staticmethod
    def get_recipe_cooking(meal):
        cooking_div = meal.find("div", class_="cooking")
        return cooking_div.text if cooking_div else ""

    async def save_recipe_picture(self, meal):
        pic_styles_div = meal.find("div", class_="food_item_img")
